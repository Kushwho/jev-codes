...
- `2026-09-19` — Jev answer values wobble ±0.02 run-to-run (case 08 leftover_debug scored 0.70 then 0.71) → keep ≥0.15 margin between a tuned threshold and the nearest negative; never tune on an exact boundary value — `jev-codes-eval`
- `2026-09-19` — duplicate_logic near-twin inversion: negative case 06 (calls the helper) scored 0.55, above positive case 04 (re-implements it) at 0.45 → no threshold separates them; threshold tuning cannot fix model-discrimination limits, log as known limitation instead of rewording off one case — `jev-codes-eval`
- `2026-09-19` — Go `log.Printf` error lines score as leftover_debug (~0.7); intentional-but-misguided logging is the main FP source for that question → threshold 0.75 (positives ≥0.95, structured-log negative 0.58) — `packs/core.yaml`
