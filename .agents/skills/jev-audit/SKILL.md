...
Invoke as `/jev-audit` with optional args (e.g. `--base <ref>`, `--pack <name>`) forwarded as `<args>` below.

## Steps

1. Run the audit via shell and parse stdout as JSON even on exit code 1. Use the first available:
